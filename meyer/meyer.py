...
# 15 mit zwei verschiedenen Zahle. Zwei bestimmte verschiedene Zahlen haben die Wahrscheinlichkeit 2/6 * 1/6 = 2/36.
# 6 als Pasch. Ein Pasch hat die Wahrscheinlichkeit 1/6 * 1/6 = 1/36.
...
class game():
    def __init__(self, ais, tout=False):
        ...
        self.meyerCount = np.zeros(self.num_of_players)
    ...
    def player_turn(self):
        if self.throw_deciders[self.current_player](self.claims):
            ...
            if self.check_meyer():
                ...
                self.meyerCount[self.current_player] += 1
                self.prep_new_round()
            else:
                self.claims.append(self.claim_deciders[self.current_player](self.claims, self.last_throw))
                self.print_out(f'roll {len(self.claims)}: player {self.current_player} rolled {self.last_throw} and claimed {self.claims[-1]}')
                self.next_player()
        else:
            self.doubt()
    
    def doubt(self):
        if self.last_throw == self.claims[-1]:
            self.points[self.current_player] += 1
...
def test_ai_throw(claims):
    if not claims:
        ...
        return True
    elif (claims[-1] >= 102) or (claims[-1] not in valid_throws) or (rank_from_throw(claims[-1]) == rank_from_throw(claims[-2])+1 ):
        return False
    else:
        return True
...
dummyAI = (dummy_throw, dummy_claims)
testAI = (test_ai_throw, test_ai_claims)
ais = [dummyAI, dummyAI, testAI]
new_game = game(ais, tout=False)
for turn in range(100000):
    new_game.player_turn()
points = new_game.return_points()
print(points/sum(points)*len(ais))
